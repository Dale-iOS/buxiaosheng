...
#import "LZZoomScrollView.h"
@interface LZPictureBrowseVC ()<UIScrollViewDelegate>
{
    UIScrollView *baseSV;
...
@implementation LZPictureBrowseVC
...
//    self.navigationItem.leftBarButtonItem  = [Utility navLeftBackBtn:self action:@selector(backMethod)];
    self.navigationItem.rightBarButtonItem = [Utility navButton:self action:@selector(toDelete:) image:IMAGE(@"del_image")];
...
- (void)viewWillAppear:(BOOL)animated {
    [self.navigationController.navigationBar setBackgroundImage:[Utility imageWithColor:[UIColor blackColor] andSize:CGSizeMake(APPWidth, (IOS7 ? 64 : 44))] forBarMetrics:UIBarMetricsDefault];
}

- (void)viewWillDisappear:(BOOL)animated {
    [self.navigationController.navigationBar setBackgroundImage:[Utility imageWithColor:[UIColor redColor] andSize:CGSizeMake(APPWidth, (IOS7 ? 64 : 44))] forBarMetrics:UIBarMetricsDefault];
}
...
- (void)setupUI {
    ...
    [_showImagesArr enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL * _Nonnull stop) {
        LZZoomScrollView *zoomScrollView = [[LZZoomScrollView alloc] initWithFrame:CGRectMake(baseSV.width*idx, 0, APPWidth, CONTENT_HEIGHT)];
...
- (void)toDelete:(id)sender {
    ...
    [baseSV.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[LZZoomScrollView class]])
        {
