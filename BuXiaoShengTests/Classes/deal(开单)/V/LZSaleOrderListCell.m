...
@implementation LZSaleOrderListCell
...
//    self.priceLabel.hidden = YES;
    ...
    [self.cancelBtn setBackgroundColor:LZAppBlueColor];
...
- (void)setModel:(LZOrderTrackingModel *)model{
    ...
    if ([_model.buyStatus integerValue] == 0 && [_model.orderStatus integerValue] == 0) {
...
- (void)cancelBtnClick{
    if ([self.delegate respondsToSelector:@selector(didCancelBtnInCell:)]) {
        [self.delegate didCancelBtnInCell:self];
    }
}
