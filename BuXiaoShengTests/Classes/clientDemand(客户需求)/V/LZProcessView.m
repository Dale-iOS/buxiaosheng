...
@interface LZProcessView()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic, strong) UITableView *tableView;
///加工类型
@property (nonatomic, strong) TextInputCell *processTypeCell;
///加工商名称
@property (nonatomic, strong) TextInputCell *processNameCell;
///联系人
...
- (void)setupFooterView
{
    ...
    //加工类型
    self.processTypeCell = [[TextInputCell alloc]initWithFrame:CGRectMake(0, lineView1.bottom, APPWidth, 49)];
    self.processTypeCell.titleLabel.text = @"加工类型";
    self.processTypeCell.contentTF.placeholder = @"请选择加工类型";
    self.processTypeCell.rightArrowImageVIew.hidden = NO;
    self.processTypeCell.contentTF.enabled = NO;
    UITapGestureRecognizer *tapProcessTypeCell = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(ProcessTypeCellClick)];
    [self.processTypeCell addGestureRecognizer:tapProcessTypeCell];
    ...
    [self.footerView addSubview:self.processTypeCell];
    [self.footerView addSubview:self.processNameCell];
...
//        _factoryLists = [LZCompanyModel LLMJParse:baseModel.data];
        ...
        [self.processNameCell.contentTF popOverSource:_factoryNameAry index:^(NSInteger index) {
            _selectedModel = [LZCompanyModel LLMJParse:_factoryListMuAry[index]];
            self.processNameCell.contentTF.text = _selectedModel.name;
...
//加工类型cell点击事件
- (void)ProcessTypeCellClick{
    [BRStringPickerView showStringPickerWithTitle:@"选择加工类型" dataSource:@[@"印花", @"复合", @"洗水", @"烫金"] defaultSelValue:nil resultBlock:^(id selectValue) {
        self.processTypeCell.contentTF.text = selectValue;
    }];
}
